...
class Body {
  ...
  offsetMomentum(px: float, py: float, pz: float): this {
    myAnalysis.propertySet<this, f64>(
      this,
      -px / SOLAR_MASS,
      "vx",
      offsetof<this>("vx")
...
class NBodySystem {
  constructor(public bodies: StaticArray<Body>) {
    ...
    for (let i = 0; i < size; ++i) {
      let b = bodies[i];
      let m = myAnalysis.propertyAccess<Body, f64>(
        b,
        "mass",
        offsetof<Body>("mass")
      );
      px +=
        myAnalysis.propertyAccess<Body, f64>(b, "vx", offsetof<Body>("vx")) * m;
      py +=
        myAnalysis.propertyAccess<Body, f64>(b, "vy", offsetof<Body>("vy")) * m;
      pz +=
        myAnalysis.propertyAccess<Body, f64>(b, "vz", offsetof<Body>("vz")) * m;
    }
    bodies[0].offsetMomentum(px, py, pz);
  }
  advance(dt: float): void {
    var bodies = myAnalysis.propertyAccess<this, StaticArray<Body>>(
      this,
      "bodies",
      offsetof<this>("bodies")
    );
    ...
    for (let i: u32 = 0; i < size; ++i) {
      ...
      let ix = myAnalysis.propertyAccess<Body, f64>(
        bodyi,
        "x",
        offsetof<Body>("x")
      );
      let iy = myAnalysis.propertyAccess<Body, f64>(
        bodyi,
        "y",
        offsetof<Body>("y")
      );
      let iz = myAnalysis.propertyAccess<Body, f64>(
        bodyi,
        "z",
        offsetof<Body>("z")
      );
      let bivx = myAnalysis.propertyAccess<Body, f64>(
        bodyi,
        "vx",
        offsetof<Body>("vx")
      );
      let bivy = myAnalysis.propertyAccess<Body, f64>(
        bodyi,
        "vy",
        offsetof<Body>("vy")
      );
      let bivz = myAnalysis.propertyAccess<Body, f64>(
        bodyi,
        "vz",
        offsetof<Body>("vz")
      );
      let bodyim = myAnalysis.propertyAccess<Body, f64>(
        bodyi,
        "mass",
        offsetof<Body>("mass")
      );
      for (let j: u32 = i + 1; j < size; ++j) {
        ...
        let dx =
          ix -
          myAnalysis.propertyAccess<Body, f64>(bodyj, "x", offsetof<Body>("x"));
        let dy =
          iy -
          myAnalysis.propertyAccess<Body, f64>(bodyj, "y", offsetof<Body>("y"));
        let dz =
          iz -
          myAnalysis.propertyAccess<Body, f64>(bodyj, "z", offsetof<Body>("z"));
        let distanceSq = dx * dx + dy * dy + dz * dz;
        let distance = <float>Math.sqrt(distanceSq);
        let mag = dt / (distanceSq * distance);
        let bim = bodyim * mag;
        let bjm =
          myAnalysis.propertyAccess<Body, f64>(
            bodyj,
            "mass",
            offsetof<Body>("mass")
          ) * mag;
      ...
      myAnalysis.propertySet<Body, f64>(
        bodyi,
        bivx,
        "vx",
        offsetof<Body>("vx")
  ...
  energy(): float {
    ...
    for (let i: u32 = 0, size: u32 = bodies.length; i < size; ++i) {
      ...
      let vx = myAnalysis.propertyAccess<Body, f64>(
        bodyi,
        "vx",
        offsetof<Body>("vx")
      );
      let vy = myAnalysis.propertyAccess<Body, f64>(
        bodyi,
        "vy",
        offsetof<Body>("vy")
      );
      let vz = myAnalysis.propertyAccess<Body, f64>(
        bodyi,
        "vz",
        offsetof<Body>("vz")
      );
      let bim = myAnalysis.propertyAccess<Body, f64>(
        bodyi,
        "mass",
        offsetof<Body>("mass")
      );
...
export function init(): void {
  system = new NBodySystem([
    (function (): Body {
      ...
      return apply0Args<Body>("Sun", changetype<usize>(Sun), args);
    })(),
    (function (): Body {
      var args = new ArgsBuffer([]);
      return apply0Args<Body>("Jupiter", changetype<usize>(Jupiter), args);
    })(),
    (function (): Body {
      var args = new ArgsBuffer([]);
      return apply0Args<Body>("Saturn", changetype<usize>(Saturn), args);
    })(),
    (function (): Body {
      var args = new ArgsBuffer([]);
      return apply0Args<Body>("Uranus", changetype<usize>(Uranus), args);
    })(),
    (function (): Body {
      var args = new ArgsBuffer([]);
      return apply0Args<Body>("Neptune", changetype<usize>(Neptune), args);
    })(),
  ]);
}
...
export function getBody(index: i32): Body | null {
  var bodies = myAnalysis.propertyAccess<NBodySystem, StaticArray<Body>>(
    system,
    "bodies",
    offsetof<NBodySystem>("bodies")
  );
